...
//		createButtons ();
	...
	{	
		...
		lbl.text = PlayerPrefs.GetInt ("NowMode") + "-" + (cLevel+1);
		

		//头图片个数
		int maxCard = Globe.askbox [cLevel].Length;	
		...
		for (int j = 0; j < maxCard; j++) {
			int __count = int.Parse (Globe.askbox [cLevel] [j]);
			if (j == maxCard - 1) {
...
//				print (temp [k]);
		...
		print ("current level is " + cLevel + " from 0 ,and findCount = " + Globe.findCount);
